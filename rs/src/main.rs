...
fn revert_mdk(rom: &mut RomBuf) {
		rom.set_bytes(0x018127, &[0xbd, 0xc8, 0x14, 0xf0]).unwrap();
		rom.set_bytes(0x018151, &[0xa9, 0xff, 0x9d, 0x1a, 0x16]).unwrap();
		rom.set_bytes(0x018172, &[0xa9, 0x08, 0x9d, 0xc8, 0x14]).unwrap();
		rom.set_bytes(0x0182b3, &[0xa7, 0x87]).unwrap();
		rom.set_bytes(0x0185c3, &[0x9c, 0x91, 0x14, 0xb5, 0x9e]).unwrap();
		rom.set_bytes(0x0187a7, &[0x22, 0x59, 0xda, 0x2]).unwrap();
		rom.set_bytes(0x01c089, &[0xbd, 0xd4, 0x14, 0x9d, 0x7b,
								  0x18, 0x29, 0x01, 0x9d, 0xd4, 0x14]).unwrap();
		rom.set_bytes(0x01c4cb, &[0xc9, 0x21, 0xd0, 0x69]).unwrap();
		rom.set_bytes(0x01c6d6, &[0xaa, 0xbd, 0x09, 0xc6]).unwrap();
		rom.set_bytes(0x01d43e, &[0x9e, 0xc8, 0x14, 0x60]).unwrap();
		rom.set_bytes(0x02a866, &[0xc9, 0xe7, 0x90, 0x22]).unwrap();
		rom.set_bytes(0x02a94b, &[0x29, 0x0d, 0x9d, 0xe0, 0x14]).unwrap();
		rom.set_bytes(0x02a963, &[0x29, 0x0d, 0x9d, 0xd4, 0x14]).unwrap();
		rom.set_bytes(0x02a9a6, &[0xaa, 0xbf, 0x59, 0xf6, 0x07]).unwrap();
		rom.set_bytes(0x02a9c9, &[0x22, 0xd2, 0xf7, 0x07]).unwrap();
		rom.set_bytes(0x02aba0, &[0xa5, 0x04, 0x38, 0xe9, 0xc8]).unwrap();
		rom.set_bytes(0x02affe, &[0xad, 0xb9, 0x18, 0xf0, 0x27]).unwrap();
		rom.set_bytes(0x02b395, &[0xbc, 0xab, 0x17, 0xf0, 0x0a]).unwrap();
